...
def merge_csv_files(directory):
    try:
        df_pae=pd.read_csv('pae_local_global.csv')
    except:
        return pd.DataFrame(columns=['plddt_binder', 'pae_interaction', 'CUTRE'])
    df_list = []
    print(df_pae)
    ...
    if df_list:
        ...
        print(merged_df)
        merged_df=pd.merge(merged_df, df_pae, on='description')
...
def update_scatter_plot(directory, plotpaethres, plotplddtthres, x_variable):
    merged_df = merge_csv_files(directory)
    if not merged_df.empty:
        filtered_df = merged_df[(merged_df['plddt_binder'] >= plotplddtthres) & (merged_df[x_variable] <= plotpaethres)]
        if len(filtered_df)!=0:
            row_count_text = f"Finished models: {len(merged_df)} | Number of hits: {len(filtered_df)} | Hit efficiency: {round(len(filtered_df)/len(merged_df)*100, 2)}% | Mean pae_interaction: {round(np.mean(filtered_df['pae_interaction']),2)} | Mean plddt_binder: {round(np.mean(filtered_df['plddt_binder']),2)} | Mean length: {round(np.mean(filtered_df['length']),0)} +- {round(np.std(filtered_df['length']))}" #standard deviation added, maybe there is something better
        else: 
        ...
        scatter_plot.update_layout(
            ...
            legend=dict(x=1, y=1),
            shapes=[
                dict(type="rect", x0=0, y0=plotplddtthres, x1=plotpaethres, y1=100, line=dict(color="black"), opacity=0.5),
        ...
        end_time4 = time.time()
        print('plotting time: ', end_time4-start_time4)
        return scatter_plot, row_count_text
    else:
        return None, "No data available."

# Function to get best hit name
def get_hit_names(df, pae_interaction_threshold, xaxis_value):
    if xaxis_value == 'pae_interaction':
        ...
        filtered_df = df[df['CUTRE'] <= pae_interaction_threshold]
        if not filtered_df.empty:
            sorted_df = filtered_df.sort_values(by='CUTRE', ascending=True)
            hit_names = sorted_df['description'].tolist()
            return hit_names
        else:
            return ["No hits found under the specified conditions"]
...
app.layout = html.Div([
    ...
    html.Button('Extract hits', id='execute-hits', n_clicks=0, style=bt_style),
    dcc.RadioItems(['PMPNN', 'AF2'],'AF2',id='extract-type', style=bt_style),
    dcc.RadioItems(['pae_interaction', 'CUTRE'], 'pae_interaction', id='xaxis_value', style=bt_style),
    dcc.Graph(id='scatter-plot', style={'width': '80vh', 'height': '70vh'}),
    html.H5('PAE and pLDDT Thresholds'),
    dcc.Input(id='filter-pae', type='text', value=pae_thres,  readOnly=False, style=box_style3),            
    dcc.Input(id='filter-plddt', type='text', value=plddt_thres, readOnly=False, style=box_style3),            
...
@app.callback(
    [Output('scatter-plot', 'figure'),
     ...
     Output('filter-pae', 'value'),
     Output('filter-plddt', 'value'),
     Output('extract-type', 'value'),],
    [Input('interval-component', 'n_intervals'),
     Input('execute-csv', 'n_clicks'),
     Input('execute-hits', 'n_clicks'),
     Input('filter-pae', 'value'),
     Input('filter-plddt', 'value'),
     Input('extract-type','value'),
     Input('xaxis_value', 'value')])

def update_graph(n, n_clicks_csv,n_clicks_hits,fpae, fplddt, extract_type,xaxis_value):
    directory='./output/'
    ...
    new_pae_thres = float(fpae)
    new_plddt_thres = float (fplddt)
    extract=str(extract_type)
    ...
    if ctx.triggered[0]['prop_id'] == 'execute-csv.n_clicks':
        if not merged_df.empty:
            ...
            if xaxis_value == 'pae_interaction':
                filtered_df = merged_df[(merged_df['plddt_binder'] >= new_plddt_thres) & (merged_df['pae_interaction'] <= new_pae_thres)]
            else:
                filtered_df = merged_df[(merged_df['plddt_binder'] >= new_plddt_thres) & (merged_df['CUTRE'] <= new_pae_thres)]
            if not filtered_df.iloc[:, 1:].empty:
                filtered_df.to_csv('hits.csv', index=False)
        scatter_plot, row_count_text = update_scatter_plot(directory,new_pae_thres,new_plddt_thres,xaxis_value)
    else:
        scatter_plot, row_count_text = update_scatter_plot(directory,new_pae_thres,new_plddt_thres,xaxis_value)
    if ctx.triggered[0]['prop_id'] == 'execute-hits.n_clicks':
        if not merged_df.empty:
            merged_df.iloc[:, 1:].to_csv('all_models.csv', index=False)
            if xaxis_value == 'pae_interaction':
                filtered_df = merged_df[(merged_df['plddt_binder'] >= new_plddt_thres) & (merged_df['pae_interaction'] <= new_pae_thres)]
            ...
            if not filtered_df.iloc[:, 1:].empty:
                ...
                for index, row in filtered_df.iterrows():
                    ...
                    hits_path = os.getcwd()+'/hits'
                    if not os.path.exists(hits_path): os.makedirs(hits_path)
    ...
    # Dropdown HITS
    dropdown_options = get_hit_names(merged_df,new_pae_thres,xaxis_value)

    return scatter_plot, row_count_text, status_df_records, job_status_counts_text, dropdown_options, new_pae_thres, new_plddt_thres, extract_type
