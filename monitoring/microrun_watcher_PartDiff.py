            ...
            for file in files:
                if file.endswith(".pdb"):
                    ...
                    with open(file_path, "r") as f:
                        # Read lines starting with "pae_" or "plddt_" and append them to the DataFrame
                        template_data = {line.strip().split()[0]: line.strip().split()[1] for line in f if line.startswith("pae_") or line.startswith("plddt_")}
            return [merged_df, template_data]
        except: print('Couldn\'t find the data for the original template!')
        return [merged_df]
    else:
...
def update_scatter_plot(directory):
    merged_df_and_reference_data = merge_csv_files(directory)
    merged_df = merged_df_and_reference_data[0]
    if not merged_df.empty:
        ...
        scatter_plot.add_scatter(
            x=[merged_df_and_reference_data[1]['pae_interaction']],
            y=[merged_df_and_reference_data[1]['plddt_binder']],
            marker=dict(
                    symbol="hexagon-dot",
...
def update_graph(n, n_clicks):
    ...
    merged_df = merge_csv_files(directory)[0]
